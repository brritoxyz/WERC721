...
import "src/interfaces/IAdminControl.sol";

import "src/interfaces/IRoyaltyRegistry.sol";
import "src/interfaces/INiftyGateway.sol";
import "src/interfaces/IFoundation.sol";
import "src/interfaces/IDigitalax.sol";
import "src/interfaces/IArtBlocks.sol";
...
contract RoyaltyRegistry is ERC165, OwnableUpgradeable, IRoyaltyRegistry {
    ...
    function setRoyaltyLookupAddress(
        ...
        require(
            tokenAddress.isContract() &&
                (royaltyLookupAddress.isContract() ||
                    royaltyLookupAddress == address(0)),
            "Invalid input"
        );
    ...
    function overrideAllowed(
        ...
        try
            IAccessControlUpgradeable(tokenAddress).hasRole(0x00, _msgSender())
        ...
        returns (address niftyRegistry) {
            try
                INiftyRegistry(niftyRegistry).isValidNiftySender(_msgSender())
            returns (bool valid) {
                return valid;
            } catch {}
        } catch {}
        ...
        returns (address payable foundationTreasury) {
            try
                IFoundationTreasury(foundationTreasury).isAdmin(_msgSender())
